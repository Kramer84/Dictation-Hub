...
class ChatMessage(BaseModel):
    ...
    name: Optional[str] = Field(None, description="Optional identifier for the participant, useful for multi-turn routing.")
...
class LLMConfig(BaseModel):
    ...
    max_tokens: Optional[int] = Field(None, description="Upper bound bound on tokens to generate.")
    top_p: float = Field(1.0, ge=0.0, le=1.0, description="Nucleus sampling probability mass threshold.")
    seed: Optional[int] = Field(None, description="Deterministic sampling seed if supported by provider backend.")
    timeout: float = Field(30.0, description="HTTP request timeout ceiling in seconds.")
    max_retries: int = Field(3, ge=0, description="Transient error connection retry threshold.")
    backoff_factor: float = Field(2.0, description="Multiplier for exponential backoff sequence calculations.")
...
class LLMProvider(ABC):
    ...
    def _execute_with_retry(self, request_fn: callable, config: LLMConfig) -> requests.Response:
        """
        ...
        while True:
            try:
                ...
                if response.status_code in [429, 500, 502, 503, 504]:
                    ...
                    logger.warning(f"Transient HTTP Status {response.status_code} observed. Retrying...")
                else:
                    response.raise_for_status()
                    return response
            except requests.exceptions.RequestException as e:
                if retries >= config.max_retries:
                    logger.error(f"Execution boundary broken. Total retries ({config.max_retries}) exhausted.")
                    raise e
    ...
    def _parse_and_validate_json(self, raw_text: str, schema: Type[T]) -> Optional[T]:
        ...
        try:
            ...
            logger.error(f"Structural validation against requested Schema definition broken: {str(json_err)}")
...
class MistralProvider(LLMProvider):
    ...
    def _build_payload(self, messages: List[ChatMessage], config: LLMConfig, response_schema: Optional[Type[BaseModel]] = None) -> Dict[str, Any]:
        payload = {
            "model": self.model,
    ...
    def generate(
        ...
        response = self._execute_with_retry(
            lambda: requests.post(self.url, headers=headers, json=payload, timeout=cfg.timeout),
            cfg
        )
        ...
        return GenerationResult(raw_content=raw_content, parsed_object=parsed_obj, metrics=metrics)
    ...
    def generate_stream(
        ...
        response = requests.post(self.url, headers=headers, json=payload, timeout=cfg.timeout, stream=True)
        response.raise_for_status()
...
class LocalProvider(LLMProvider):
    ...
    def generate(
        ...
        response = self._execute_with_retry(
            lambda: requests.post(self.endpoint, headers=headers, json=payload, timeout=cfg.timeout),
            cfg
        )
    ...
    def generate_stream(
        ...
        headers = {"Content-Type": "application/json"}
        response = requests.post(self.endpoint, headers=headers, json=payload, timeout=cfg.timeout, stream=True)
        response.raise_for_status()
...
if __name__ == "__main__":
    ...
    class EntityExtraction(BaseModel):
        ...
        valuation_billion: float = Field(..., description="Market valuation listed in billions USD.")
        core_technology: str = Field(..., description="The key domain focus of the organization.")
        tags: List[str] = Field(default_factory=list, description="Keywords summarizing the entity.")

    # Frame conversation execution parameters
    pipeline_context = [
        ChatMessage(role="system", content="You are a strict financial extraction engine. Output your answers in complete compliance with schemas asked."),
        ChatMessage(role="user", content="Extract details from this notice: 'Yesterday, AcmeAI closed its funding round valuing the startup at $4.2 billion. Their transformer core framework remains unmatched.'")
    ]
