...
class StorageConfig(BaseModel):
    ...
    base_dir: Path = Field(..., description="The root directory for saving transcriptions.")
    folder_format: str = Field(..., description="Datetime format string for generating subfolders.")
...
class SuffixesConfig(BaseModel):
    ...
    full_json: str = Field(..., description="Suffix for the complete Whisper JSON payload.")
    cleaned_json: str = Field(..., description="Suffix for the deterministic cleaned JSON output.")
    cleaned_md: str = Field(..., description="Suffix for the markdown representation of cleaned segments.")
    raw_text: str = Field(..., description="Suffix for the flattened string footprint of the dictation.")
...
class WhisperPipelineConfig(BaseModel):
    ...
    def load_from_file(cls, file_path: Union[str, Path]) -> "WhisperPipelineConfig":
        ...
        if not path.is_file():
            raise FileNotFoundError(f"Configuration file could not be found at: {path.absolute()}")
        ...
        try:
            ...
            raise ValueError(f"Failed to parse JSON file at {path}. Invalid JSON structure: {str(e)}")
        except ValidationError as e:
            raise ValueError(f"Configuration validation failed. The JSON schema does not match the expected structure:\n{e}")