...
class ScheduleEvent(BaseModel):
    ...
    end_datetime: Optional[str] = Field(
        None, description="ISO 8601 format. If a broad block like 'afternoon' is dictated, set to 18:00:00. Otherwise, leave null and the system will auto-calculate it based on duration."
    )
    duration_minutes: int = Field(
        60, description="Duration in minutes. Defaults to 60 for specific events. For broad blocks (e.g., afternoon), calculate total span (e.g., 300)."
    )
    ...
    def parse_and_format_iso(cls, v):
        ...
        try:
            ...
            if "Temporal logic failure" in str(e):
                raise e
            raise ValueError(f"Invalid ISO 8601 format: '{v}'. Expected format is YYYY-MM-DDTHH:MM:SS.")
    ...
    def sync_duration_and_endtime(self):
        ...
        if dt_end < dt_start:
            raise ValueError("Logical error: 'end_datetime' cannot occur before 'start_datetime'.")
        
        # 4. Rule B: Contextual integrity
        if self.intent_type == "date_range" and (dt_end - dt_start).total_seconds() < 86400:
    ...
    def check_cross_field_logistics(self):
        ...
        if self.intent_type == "date_range" and not self.end_datetime:
            raise ValueError("Logical error: 'date_range' intent strictly requires an 'end_datetime'.")
    ...
    def execute(self, input_json_path: Path) -> str:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                print("[SchedulingPipeline] JSON Schema and Logistic Business Logic validated successfully.")
