...
def load_env_config(config_path: Path) -> dict[str, str]:
    """
    Parses a simple key=value bash-like configuration file.
...
def apply_config_overrides(config_path: Path, current_args: dict[str, Any]) -> dict[str, Any]:
    """
    Updates the default CLI arguments with values loaded from a configuration file.
    ...
    current_args : dict[str, Any]
        The dictionary of current arguments and their values.

    Returns
    -------
    dict[str, Any]
        A new dictionary with updated values based on the configuration file.
    """
    ...
    for key, val in env_vars.items():
        ...
        if lower_key in updated_args:
            ...
            # Type-cast the string config value to match the parameter's original type
            if isinstance(original_value, bool):
                ...
                updated_args[lower_key] = Path(val)
            else:
...
def resolve_paths(
    ...
    """
    Validates and resolves paths for the input file, executable, models, and outputs.
    ...
    whisper_dir : Path
        The base directory containing the whisper.cpp installation.
    model : str
        The whisper model name to use.
    ...
    -------
    tuple[Path, Path, Path]
        A tuple containing the resolved paths: (cli_exec, model_path, resolved_output_base).

    Raises
    ------
    FileNotFoundError
        If the input audio file or the whisper-cli executable does not exist.
    """
    ...
    if not cli_exec.is_file() and not os.access(cli_exec, os.X_OK):
        raise FileNotFoundError(f"whisper-cli executable not found or not executable at {cli_exec}")
...
def build_whisper_command(args: dict[str, Any]) -> list[str]:
    """
    Constructs the list of command line arguments to pass to the whisper-cli executable.

    Parameters
    ----------
    args : dict[str, Any]
        The dictionary of resolved arguments for the transcription.

    Returns
    -------
    list[str]
        A list of string arguments ready to be passed to subprocess.run().
    """
    # Base command formulation
    cmd = [
        str(args["cli_exec"]),
        "-f", str(args["input_wav"]),
        ...
        "-et", str(args["entropy_thold"]),
        "-lpt", str(args["logprob_thold"]),
        "-wt", str(args["word_thold"]),
        "-l", str(args["language"]),
        "-ot", str(args["offset_t"]),
        "-on", str(args["offset_n"]),
        "-d", str(args["duration"]),
        "-ml", str(args["max_len"]),
        "-bo", str(args["best_of"]),
        "-nth", str(args["no_speech_thold"]),
        "--temperature", str(args["temperature"]),
        "--temperature-inc", str(args["temperature_inc"]),
        "--ov-e-device", str(args["ov_e_device"]),
        "--grammar-penalty", str(args["grammar_penalty"]),
    ]
    ...
    if args.get("use_vad"):
        vad_model_path = args["whisper_dir"] / "models" / f"ggml-{args['vad_model']}.bin"
        if vad_model_path.is_file():
            cmd.extend([
                "--vad",
                "--vad-model", str(vad_model_path),
                "--vad-threshold", str(args["vad_thold"]),
                "--vad-min-speech-duration-ms", str(args["vad_min_speech"]),
                "--vad-min-silence-duration-ms", str(args["vad_min_silence"]),
                "--vad-max-speech-duration-s", str(args["vad_max_speech"]),
                "--vad-speech-pad-ms", str(args["vad_pad"]),
                "--vad-samples-overlap", str(args["vad_overlap"]),
            ])
...
def whisper_transcribe(
    ...
    input_wav: Annotated[Path, typer.Option("--input", help="A valid input audio file")],
    config: Annotated[Optional[Path], typer.Option("--config", help="Environment config file to override defaults")] = None,
    output_base: Annotated[Optional[Path], typer.Option("--output", help="Output base filepath")] = None,
    
    # --- Core Options ---
    whisper_dir: Annotated[Path, typer.Option(help="Directory containing whisper.cpp base")] = Path.home() / "whisper.cpp",
    ...
    duration: Annotated[int, typer.Option(help="Duration of audio to process in ms (0=all)")] = 0,
    max_context: Annotated[int, typer.Option(help="Maximum context length")] = -1,
    max_len: Annotated[int, typer.Option(help="Maximum segment length in characters")] = 0,
    split_on_word: Annotated[bool, typer.Option(help="Split on word rather than on token")] = False,
    ...
    word_thold: Annotated[float, typer.Option(help="Word timestamp probability threshold")] = 0.01,
    entropy_thold: Annotated[float, typer.Option(help="Entropy threshold for decoder fail")] = 2.40,
    logprob_thold: Annotated[float, typer.Option(help="Log probability threshold for decoder fail")] = -1.00,
    no_speech_thold: Annotated[float, typer.Option(help="No-speech probability threshold")] = 0.60,
    temperature: Annotated[float, typer.Option(help="Initial decoding temperature")] = 0.00,
    temperature_inc: Annotated[float, typer.Option(help="Temperature increment step")] = 0.20,

    # --- Modes & Behaviours ---
    debug_mode: Annotated[bool, typer.Option(help="Enable debug mode")] = False,
    translate: Annotated[bool, typer.Option(help="Translate from source language to English")] = False,
    ...
    no_fallback: Annotated[bool, typer.Option(help="Do not use temperature fallback")] = False,
    ...
    output_words: Annotated[bool, typer.Option(help="Output per-word timestamps")] = False,
    ...
    output_json_full: Annotated[bool, typer.Option(help="Output full JSON file")] = True,

    # --- Printing Preference ---
    no_prints: Annotated[bool, typer.Option(help="Do not print transcription progress")] = True,
    print_special: Annotated[bool, typer.Option(help="Print special tokens")] = False,
    print_colors: Annotated[bool, typer.Option(help="Print colors for transcription")] = False,
    print_progress: Annotated[bool, typer.Option(help="Print progress")] = False,
    no_timestamps: Annotated[bool, typer.Option(help="Do not print timestamps")] = False,

    # --- Language Setup ---
    language: Annotated[str, typer.Option(help="Language code (e.g., 'en', 'auto')")] = "auto",
    detect_language: Annotated[bool, typer.Option(help="Automatically detect language")] = False,
    initial_prompt: Annotated[str, typer.Option(help="Initial prompt for the model")] = "",
    ...
    suppress_nst: Annotated[bool, typer.Option(help="Suppress non-speech tokens")] = True,
    suppress_regex: Annotated[str, typer.Option(help="Regex expression to suppress")] = "",
    ...
    vad_model: Annotated[str, typer.Option(help="Silero VAD model name")] = "silero-v6.2.0",
    vad_thold: Annotated[float, typer.Option(help="VAD threshold")] = 0.60,
    vad_min_speech: Annotated[int, typer.Option(help="VAD min speech duration ms")] = 100,
    vad_min_silence: Annotated[int, typer.Option(help="VAD min silence duration ms")] = 100,
    ...
    """
    Main entry point for the Whisper CLI wrapper. Orchestrates path validations,
    configuration overrides, command building, and subprocess execution.
    """
    # Capture all local variables corresponding to command-line parameters
    current_args = locals().copy()

    # Apply configuration file overrides if provided
    if config:
        current_args = apply_config_overrides(config, current_args)

    try:
        # Resolve required operational paths
        ...
        # Merge resolved path artifacts back into kwargs for the builder
        current_args.update({
            "cli_exec": cli_exec,
    ...
    if result.returncode == 0:
        typer.secho(f"✅ Transcription process complete using target: {resolved_output_base}", fg=typer.colors.GREEN)
    else:
